...
you can skip the first column using the skip argument in read_excel
...
you can supply a `col_types` argument to [`read_excel()`](https://readxl.tidyverse.org/reference/read_excel.html) and specify the column types for the variables you read in. The syntax is a bit different, though. Your options are `"skip"`, `"guess"`, `"logical"`, `"numeric"`, `"date"`, `"text"` or `"list"`.
...
You can read a single worksheet from a spreadsheet with the `sheet` argument in [`read_excel()`](https://readxl.tidyverse.org/reference/read_excel.html)
...
1.Then, read it into R, with `survey_id` as a character variable and `n_pets` as a numerical variable
...
2.Then, read it into R. The resulting data frame should be called `roster` and should look like the following
