...
This plot shows the number one song in each year, this plot akso shows that as the years go on song popularity rises through the 2000s.
...
This shows that there is indeed a difference in popularity in our two song sets. This help validate our approach of using these two song sets to examine what variables might determine song popularity.
...
The set of plots below are comparing the popular and the unpopular songs to see if any of them have any noticeable differences but there actually is much of a difference between unpopular and popular songs for any of these variables.
...
The above plots show that there's really no noticeable difference in each category, you would think that tempo would be different but it looks like unpopular songs try to mimic popular songs.
...
pip <- full_join(trash_songs_df, all_songs_df, by = "track.id")

pip <- bind_rows(list(unpopular = trash_songs_df, popular = good_songs_df), 
                          .id= "song_group")
